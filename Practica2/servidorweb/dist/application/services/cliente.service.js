import { CategoriaCliente } from '../../domain/cliente/cliente.entity.js';
...
export class ClienteService {
     ...
     */
    async obtenerClientePorDocumento(tipoDocumento, numeroDocumento) {
     ...
     */
    async obtenerClientesPorEstado(estado) {
     ...
     */
    async obtenerClientesPorCategoria(categoria) {
     ...
     */
    async ascenderCategoriaCliente(id) {
        const cliente = await this.clienteRepository.obtenerPorId(id);
        if (!cliente) {
            return {
                success: false,
     ...
     */
    async verificarDisponibilidadDocumento(tipoDocumento, numeroDocumento) {
     ...
     */
    async obtenerClientesMayoresDeEdad() {
    ...
    async obtenerClientesVIP() {
        const todosClientes = await this.clienteRepository.obtenerTodos();
        return todosClientes.filter(cliente => cliente.categoria === CategoriaCliente.ORO ||
