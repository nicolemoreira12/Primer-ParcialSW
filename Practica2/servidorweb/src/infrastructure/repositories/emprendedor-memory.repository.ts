 ...
 * - DELETE: Async/Await
 */
export class EmprendedorMemoryRepository implements IEmprendedorRepository {
    private emprendedores: Map<string, Emprendedor> = new Map();
    private readonly LATENCIA_RED = 150; // Simular latencia de red
        ...
        setTimeout(() => {
            try {
                ...
                const emprendedor = new Emprendedor(
                    ...
                    emprendedorData.nombre,
                    emprendedorData.apellido,
                    email,
                    emprendedorData.telefono,
                    especialidad,
                    biografia,
                    emprendedorData.sector,
                    emprendedorData.experiencia,
                    emprendedorData.usuarioId,
                    redesSociales
                );
     ...
     */
    async obtenerPorId(id: string): Promise<Emprendedor | null> {
        await this.simularLatencia();
        return this.emprendedores.get(id) || null;
    }

    async obtenerPorEmail(email: string): Promise<Emprendedor | null> {
        await this.simularLatencia();
    ...
    async obtenerPorUsuarioId(usuarioId: string): Promise<Emprendedor | null> {
        await this.simularLatencia();
    ...
    async obtenerTodos(): Promise<Emprendedor[]> {
        await this.simularLatencia();
        return Array.from(this.emprendedores.values());
    }

    async obtenerPorEstado(estado: EstadoEmprendedor): Promise<Emprendedor[]> {
        await this.simularLatencia();
    ...
    async obtenerPorSector(sector: SectorEmprendimiento): Promise<Emprendedor[]> {
        await this.simularLatencia();
    ...
    async obtenerPorExperiencia(experiencia: NivelExperiencia): Promise<Emprendedor[]> {
        await this.simularLatencia();
    ...
    async obtenerVerificados(): Promise<Emprendedor[]> {
        await this.simularLatencia();
    ...
    async contarPorSector(sector: SectorEmprendimiento): Promise<number> {
        await this.simularLatencia();
    ...
    async contarVerificados(): Promise<number> {
        await this.simularLatencia();
    ...
    async obtenerPuntuacionPromedio(): Promise<number> {
        await this.simularLatencia();

        const emprendedores = Array.from(this.emprendedores.values());
        if (emprendedores.length === 0) return 0;
    ...
    private inicializarDatosPrueba(): void {
        try {
            ...
            const empTech = new Emprendedor(
                'emp-001',
                'Laura',
                'Martínez',
                new Email('laura.martinez@techstartup.com'),
                '+57-315-7894561',
                new Especialidad('Desarrollo de Software'),
                new Biografia('Desarrolladora Full Stack con 8 años de experiencia en crear soluciones tecnológicas innovadoras para startups. Especializada en React, Node.js y arquitecturas cloud. Fundadora de 3 empresas exitosas en el sector fintech.'),
                SectorEmprendimiento.TECNOLOGIA,
                ...
                EstadoEmprendedor.VERIFICADO,
                4.8
            );

            // Emprendedor de Alimentación
            const empFood = new Emprendedor(
                'emp-002',
                'Diego',
                'Ramírez',
                new Email('diego.ramirez@foodie.com'),
                '+57-300-1122334',
                new Especialidad('Chef y Creación de Productos Alimenticios'),
                new Biografia('Chef profesional con experiencia en restaurantes de alta cocina. Especializado en comida saludable y sostenible, con certificaciones internacionales en nutrición y manejo de alimentos orgánicos.'),
                SectorEmprendimiento.ALIMENTACION,
                ...
                EstadoEmprendedor.ACTIVO,
                4.5
            );

            // Emprendedor de Moda Emergente
            const empFashion = new Emprendedor(
                'emp-003',
                'Camila',
                'Torres',
                new Email('camila.torres@fashiondesign.com'),
                '+57-310-5566778',
                new Especialidad('Diseño de Moda Sostenible'),
                new Biografia('Diseñadora de modas enfocada en crear piezas únicas utilizando materiales reciclados y técnicas artesanales tradicionales colombianas. Graduada de diseño con especialización en sostenibilidad.'),
                SectorEmprendimiento.MODA,
                ...
                EstadoEmprendedor.PENDIENTE_VERIFICACION,
                4.2
            );
    ...
    obtenerEstadisticas(): {
        ...
        const porExperiencia: Record<string, number> = {};
